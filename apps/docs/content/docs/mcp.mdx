...
- package: `@deepagent/mcp`
- exports: `createMcpTools`, `class McpTools`
- [source](https://github.com/amardeeplakshkar/deepagent/blob/main/packages/mcp/src/mcp-tools.ts)
...
{`
\`\`\`bash npm
npm install @deepagent/mcp
\`\`\`

\`\`\`bash yarn
yarn add @deepagent/mcp
\`\`\`

\`\`\`bash pnpm
pnpm add @deepagent/mcp
\`\`\`
`}
</PackageTabs>
...
import { createAISDKTools } from '@deepagent/ai-sdk'
import { createMcpTools } from '@deepagent/mcp'
...
`createMcpTools` takes in the following options ([source](https://github.com/amardeeplakshkar/deepagent/blob/main/packages/mcp/src/types.ts)):
